...
describe('Calculadora - Operação de Soma', () => {
  ...
  test('soma números decimais corretamente', () => {
    expect(calculadora.somar(1.5, 2.3)).toBeCloseTo(3.8);
    expect(calculadora.somar(0.1, 0.2)).toBeCloseTo(0.3);
  });

  test('lança erro com argumento não numérico', () => {
    expect(() => calculadora.somar('a', 2)).toThrow('Argumentos devem ser números');
  });

  test('lança erro com dois argumentos não numéricos', () => {
    expect(() => calculadora.somar('a', 'b')).toThrow('Argumentos devem ser números');
  });
});
